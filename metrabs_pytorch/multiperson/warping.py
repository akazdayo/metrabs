...
def warp_images_with_pyramid(
    ...
    intrinsic_matrix_levels = [
        corner_aligned_scale_mat(1 / 2**i_level).to(intrinsic_matrix.device)
        @ intrinsic_matrix
...
def warp_single_image(
    image, intrinsic_matrix, new_invprojmat, distortion_coeffs, output_shape
):
    device = image.device
    ...
    return torch.nn.functional.grid_sample(
        image.unsqueeze(0),
        old_coords_normalized.unsqueeze(0),
        align_corners=True,
        mode="bilinear",
        padding_mode="zeros",
    ).squeeze(0)
...
def distortion_formula_parts(undist_points2d, distortion_coeffs):
    ...
    broadcast_shape = (
        ([-1] if d.ndim > 1 else []) + [1] * (undist_points2d.ndim - d.ndim) + [12]
    )
...
def corner_aligned_scale_mat(factor):
    ...
    return torch.from_numpy(
        np.array([[factor, 0, shift], [0, factor, shift], [0, 0, 1]], dtype=np.float32)
    )
